...
func main() {
	...
	r.HandleFunc("/consent", sessionCheckMiddleware(consentHandler)).Methods("GET")
	r.HandleFunc("/generate_access_token", sessionCheckMiddleware(generateAccessTokenHandler)).Methods("POST")
...
// consentHandler implements the consent endpoint.
// Ref: https://developers.google.com/privacy-sandbox/blog/fedcm-chrome-126-updates#continuation-api
func consentHandler(w http.ResponseWriter, r *http.Request) {
	session, _ := store.Get(r, "session")
	scope := session.Values["scope"]
	scopeStr, _ := scope.(string)
	fields := session.Values["fields"]
	fieldsStr, _ := fields.(string)
	data := struct {
		Scope  string
		Fields string
	}{
	...
	renderTemplate(w, "consent.html", data)
}

// generateAccessTokenHandler implements the generate_access_token endpoint.
// Ref: https://developers.google.com/privacy-sandbox/blog/fedcm-chrome-126-updates#continuation-api
func generateAccessTokenHandler(w http.ResponseWriter, r *http.Request) {
	slog.Info("Generate access token")
...
func fedcmAssertionHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Ref: https://developers.google.com/privacy-sandbox/blog/fedcm-chrome-126-updates#parameters-api
	if r.FormValue("param_scope") != "" {
		sess.Values["scope"] = r.FormValue("param_scope")
		slog.Info("Scope requested", "scope", r.FormValue("param_scope"))
	}

	// Ref: https://developers.google.com/privacy-sandbox/blog/fedcm-chrome-126-updates#fields-api
	if r.FormValue("fields") != "" {
		sess.Values["fields"] = r.FormValue("fields")
		slog.Info("Fields requested", "fields", r.FormValue("fields"))
	}
	...
	jsonResponse(w, map[string]string{"continue_on": "/consent"}, http.StatusOK)
}
